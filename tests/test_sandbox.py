...
def test_allowed_tools_exact():
    ...
    # recall_topo=Phase 4.6 拓扑记忆脑区)在 union;code agent 的 system prompt 不列它们(不泄漏),
    # 仅幻觉调用时触发 → dispatch 显式报错。
    assert ALLOWED_TOOLS == frozenset(
        {"read_text", "search_text", "inspect_file", "apply_text_patch", "workspace_run_check",
         "list_allowed_roots", "observe", "act", "recall_map", "plan", "recall_topo"}
    )
